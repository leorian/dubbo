...
public class RegistryServerSync implements InitializingBean, DisposableBean, NotifyListener {
    ...
    public void notify(List<URL> urls) {
        ...
        if (urls.get(0).getPath().contains("com.bozhong.boss.service.PartnerServiceRPC")) {
            System.out.println("哈哈：" + urls);
        }
        // Map<category, Map<servicename, Map<Long, URL>>>
        final Map<String, Map<String, Map<Long, URL>>> categories = new HashMap<String, Map<String, Map<Long, URL>>>();
        Set<String> serviceInterfaceNames = new HashSet();
        for (URL url : urls) {
            serviceInterfaceNames.add(url.getParameter(Constants.GROUP_KEY) + "/" + url.getServiceInterface());
        ...
        for (String category : categories.keySet()) {
            ...
            Set<String> overrideServiceKeys = new HashSet<String>();
            for (String serviceKey : serviceKeys) {
                for (String serviceInterfaceName : serviceInterfaceNames) {
                    if (serviceKey.startsWith(serviceInterfaceName)) {
                        overrideServiceKeys.add(serviceKey);
                    }
                }
            }
            for (String overrideService : overrideServiceKeys) {
                services.remove(overrideService);
            }
        }
